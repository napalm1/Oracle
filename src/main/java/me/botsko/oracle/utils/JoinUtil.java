...
public class JoinUtil {
	...
	public static void registerPlayerJoin( String player, String ip, int online_count ){
		try {
	        ...
	        PreparedStatement s = conn.prepareStatement("INSERT INTO oracle_joins (player_count,player,player_join,ip) VALUES (?,?,now(),?)");
	...
	public static void registerPlayerQuit( String player ){
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("UPDATE oracle_joins SET player_quit = now() WHERE player_quit IS NULL AND player = ?");
			...
			while( trs.next() ){
				PreparedStatement pstmt1 = conn.prepareStatement("UPDATE oracle_joins SET playtime = ? WHERE id = ?");
	...
	public static void forceDateForOfflinePlayers( String users ){
		try {
			...
			// Log as having quit
	        PreparedStatement s = conn.prepareStatement( "UPDATE oracle_joins SET player_quit = now() WHERE player_quit IS NULL"+users );
    		s.executeUpdate();
